...
在调用read_six_numbers()之后，读入的六个整数被存放在%rsp到0x18(%rsp)之间（每四个字节存一个数），随后取出%r13指向位置的数字将其减1后与5比大小，在小于等于时跳转，否则爆炸。说明M[R[r13]]小于等于6

跳转后将%r12d中的数字（初始是0）加1与6比大小，相等则跳转（此处相当于%r12d是循环变量，边界条件是6，要执行六次循环）。若未跳转，将循环变量的值存入%ebx，并移入%rax中，将第%rax个读入的数字存入%eax与(%rbp)比较，不相等则跳转，否则爆炸。说明每一个数都和%rbp指向的位置上的数字不同。

跳转后ebx+1和5比大小，在小于等于5时跳转回ebx存入rax那一步（遍历后5个数字，全部要和rbp中的数字不同）

随后将%r13加4，存入rbp中（指向第二个读入的数字），
